...
class FirebaseRestAPI:
    ...
    # ============================================================
    # USER METHODS - FULL SYNC
    # ============================================================
    def debug_sync_material(self, page: ft.Page, material_id):
        """Debug sync a specific material"""
        import sqlite3
        from database import DB_PATH
        ...
        # Print debug info
        print(f"📤 DEBUG: Material {material_id} data:")
        for key, value in material_dict.items():
            print(f"  {key}: {value} (type: {type(value).__name__})")
            if value is None:
                print(f"    ⚠️ WARNING: {key} is None")
        ...
        if result:
            page.snack_bar = ft.SnackBar(
                ft.Text(f"✅ Material {material_dict.get('name')} synced successfully!"),
                bgcolor=self.success_color,
                duration=3000
            )
        else:
            page.snack_bar = ft.SnackBar(
                ft.Text(f"❌ Failed to sync material {material_dict.get('name')}"),
                bgcolor=self.danger_color,
                duration=3000
            )
        page.snack_bar.open = True
    ...
    def sync_user_full(self, company_id, user_data):
        """Sync user with ALL fields - FIXED for Firebase compatibility"""
        ...
        try:
            ...
            document = {
                "fields": {
                    ...
                    "name": {"stringValue": safe_string(user_data.get('name'))[:100]},
                    "email": {"stringValue": safe_string(user_data.get('email'))[:100]},
            ...
            print(f"Sync user full error: {e}")
            return False
    ...
    # ============================================================
    # MATERIAL METHODS - FULL SYNC
    # ============================================================
    
    def sync_material_full(self, company_id, material_data):
        """Sync material with ALL fields - FIXED for Firebase compatibility"""
        if not self.is_ready():
            return False
        ...
        try:
            ...
            document = {
                "fields": {
                    ...
                    "name": {"stringValue": safe_string(material_data.get('name'))[:100]},  # Limit length
                    ...
                    "location_ids": {"stringValue": safe_string(material_data.get('location_ids'))[:200]},
                    "size": {"stringValue": safe_string(material_data.get('size'))[:50]},
                    "length": {"stringValue": length_str},
                    "colors": {"stringValue": safe_string(material_data.get('colors'))[:100]},
                    "notes": {"stringValue": safe_string(material_data.get('notes'))[:500]},
                    "barcode_value": {"stringValue": safe_string(material_data.get('barcode_value'))[:50]},
                    "image_path": {"stringValue": safe_string(material_data.get('image_path'))[:200]},
            ...
            # Send to Firebase with debug
            print(f"📤 Uploading material {material_id}: {material_data.get('name')}")
            ...
            if response.status_code in [200, 201]:
                ...
                # Try to print the error details
                try:
            ...
            print(f"Sync material full error: {e}")
            import traceback
            traceback.print_exc()
            return False
        ...
        try:
            ...
            for doc in data.get('documents', []):
                ...
                material = {
                    ...
                    'length': float(fields.get('length', {}).get('stringValue', 0)) if fields.get('length', {}).get('stringValue') else None,
    ...
    # ============================================================
    # ACCESSORY METHODS - FULL SYNC
    # ============================================================
    
    def sync_accessory_full(self, company_id, accessory_data):
        """Sync accessory with ALL fields - FIXED for Firebase compatibility"""
        if not self.is_ready():
            return False
        ...
        try:
            ...
            document = {
                "fields": {
                    ...
                    "name": {"stringValue": safe_string(accessory_data.get('name'))[:100]},
                    ...
                    "location": {"stringValue": safe_string(accessory_data.get('location'))[:200]},
                    "notes": {"stringValue": safe_string(accessory_data.get('notes'))[:500]},
                    "barcode_value": {"stringValue": safe_string(accessory_data.get('barcode_value'))[:50]},
                    "image_path": {"stringValue": safe_string(accessory_data.get('image_path'))[:200]},
                    "company_id": {"integerValue": str(safe_int(company_id))},
                    "synced_at": {"stringValue": datetime.now().isoformat()}
                }
            }
            ...
            print(f"Sync accessory full error: {e}")
            import traceback
            traceback.print_exc()
            return False
...
class ActivationManager:
    """Manages activation codes for customers who buy the app"""
    ...
    @staticmethod
    def create_activation(customer_name, customer_email, company_name):
        """Create a new activation for a customer"""
        import sqlite3
        from database import DB_PATH
        ...
        # Generate all codes
        activation_code = ActivationManager.generate_activation_code()
        login_code = ActivationManager.generate_login_code()
        temp_password = ActivationManager.generate_temp_password()
        company_id = ActivationManager.generate_company_id()
        ...
        cursor.execute('''
            INSERT INTO activation_codes (
                code, company_name, customer_name, customer_email,
                login_code, temp_password, company_id,
                created_at, expires_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            activation_code, company_name, customer_name, customer_email,
            login_code, temp_password, company_id,
            created_at, expires_at
        ))
    ...
    def verify_activation_code(code):
        ...
        cursor.execute('''
            SELECT id, company_id, company_name, customer_name, customer_email,
                   login_code, temp_password, used, expires_at
        ...
        (activation_id, company_id, company_name, customer_name, 
         customer_email, login_code, temp_password, used, expires_at) = result
    ...
    def get_all_activations():
        ...
        cursor.execute('''
            SELECT id, code, company_name, customer_name, customer_email,
                   login_code, temp_password, company_id, used, 
...
class CloudSyncManager:
    ...
    def sync_users_to_cloud(company_id):
        """Sync users to cloud - DELETES cloud users not in local"""
        try:
    ...
    def download_users_from_cloud(company_id):
        ...
        try:
            ...
            for user in cloud_users:
                user_id = user.get('id')
                password_hash = existing.get(user_id, DEFAULT_PASSWORD)
    ...
    def sync_materials_to_cloud(company_id):
        """Sync materials to cloud - DELETES cloud materials not in local"""
        try:
            conn = sqlite3.connect("store_management.db")
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            ...
            # ===== STEP 2: FIND MATERIALS TO DELETE (in cloud but NOT in local) =====
            ...
            # ===== STEP 4: UPLOAD LOCAL MATERIALS TO CLOUD =====
            uploaded_count = 0
    ...
    def sync_accessories_to_cloud(company_id):
        """Sync accessories to cloud - DELETES cloud accessories not in local"""
        try:
            conn = sqlite3.connect("store_management.db")
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            ...
            for accessory in local_accessories:
                if firebase_api.sync_accessory_full(company_id, dict(accessory)):
                    uploaded_count += 1
    ...
    def full_sync_to_cloud(company_id):
        ...
        try:
            ...
            print(f"✅ Full sync complete: Users={user_result}, Materials={material_result}, Accessories={accessory_result}")
...
class StoreApp:
        ...
        def do_sync():
            try:
                ...
                if upload_result or download_result:
                    page.snack_bar = ft.SnackBar(
                        ft.Text(f"✅ Sync completed! Data updated."),
                        bgcolor=self.success_color,
                        duration=3000
                    )
                else:
    ...
    def show_activation_dialog(self, page: ft.Page):
        """Show activation dialog for user to enter email + activation code"""
        ...
        code_field = ft.TextField(
            ...
            hint_text="Enter code sent after purchase",
            width=350,
            bgcolor="#2C2C2C",
            border_color="#FF9800",
        ...
        def activate(e):
            ...
            # Verify code
            result, message = ActivationManager.verify_activation_code(code)
            
            if result:
                # Activation successful
                loading_indicator.visible = False
                status_text.value = "✅ Activation successful!"
                status_text.color = self.success_color
                
                # Save activation
                self.save_activation_info(email, code)
                ...
                page.snack_bar = ft.SnackBar(
                    ft.Text(f"✅ Full version activated! Welcome {email}!"),
                    bgcolor=self.success_color,
                    duration=4000
                )
                page.snack_bar.open = True
        ...
        dialog_content = ft.Column([
            ft.Row([
                ft.Text("🔑 Activate Full Version", size=20, weight=ft.FontWeight.BOLD, expand=True),
                ft.IconButton(icon=ft.icons.CLOSE, icon_size=20, on_click=close_dialog),
            ]),
            ft.Divider(),
            ft.Text("Enter the activation code sent to you after purchase:", size=13, color="#888888"),
            ft.Container(height=10),
            email_field,
            ft.Container(height=10),
            code_field,
            ...
            ft.Row([
                ...
                ft.FilledButton("Activate", on_click=activate, 
                            style=ft.ButtonStyle(bgcolor="#4CAF50", color="white"), expand=True),
            ], spacing=10),
            ft.Divider(),
            ft.Text("💡 Need an activation code? Contact support@storemanagement.com", size=10, color="#888888"),
        ], spacing=8)
        
        dialog = ft.AlertDialog(
            title=ft.Text(""),
            content=ft.Container(content=dialog_content, width=400, height=450, padding=20),
            modal=True,
        )
    ...
    def ensure_demo_users(self):
        """Ensure demo users exist - CLASS METHOD"""
        import sqlite3
        import hashlib
        from database import DB_PATH
        from datetime import datetime
            ...
            def do_sync():
                try:
                    ...
                    if success:
                        ...
                        print(f"⚠️ Auto-sync had issues for company {company_id}")
                except Exception as e:
                    print(f"Auto-sync error: {e}")
            
            threading.Thread(target=do_sync, daemon=True).start()

    def on_admin_login(self, e):
        """Auto-login with admin credentials - USES CLASS METHOD"""
        page = self.page_ref
        ...
        # ✅ Now works - class method
        self.ensure_admin_user()
        
        email = "admin@store.com"
        password = "admin123"
        
        print(f"🔐 Attempting admin login: {email}")
        ...
        if user:
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text(f"✅ Welcome Admin {user_dict.get('name', 'Admin')}!"),
                bgcolor=self.accent_color,
                duration=3000
            )
            page.snack_bar.open = True
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text("❌ Admin login failed. Please check credentials."),
                bgcolor=self.danger_color,
                duration=3000
            )
            page.snack_bar.open = True
    ...
    def on_demo_login(self, e):
        ...
        print(f"🔐 Attempting demo login: {email}")
        ...
        if user:
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text(f"✅ Welcome {user_dict.get('name', 'Demo User')}!"),
                bgcolor=self.success_color,
                duration=3000
            )
            page.snack_bar.open = True
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text("❌ Demo login failed. Please try again."),
                bgcolor=self.danger_color,
                duration=3000
            )
            page.snack_bar.open = True
    ...
    def ensure_all_users_have_codes(self):
        ...
        try:
            ...
            # Find users without login codes
            cursor.execute("SELECT id, name, email FROM users WHERE login_code IS NULL")
    ...
    def confirm_logout(self, page: ft.Page):
        """Show logout confirmation dialog and clear session"""
        ...
        dialog_content = ft.Column([
            ...
            ft.Text("You will need to login again next time.", size=12, color="#888888"),
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Container(content=dialog_content, width=350, height=320, padding=20),
        )
    ...
    def clear_session(self):
        """Clear saved session - CLASS METHOD"""
        import os
        try:
    ...
    def save_activation_info(self, email, activation_code):
        ...
        try:
            ...
            # Check if trial_info exists
            cursor.execute("SELECT id FROM trial_info LIMIT 1")
            result = cursor.fetchone()
            
            if result:
                cursor.execute('''
                    UPDATE trial_info 
                    SET email = ?, activated = 1, activation_code = ?
                    WHERE id = ?
                ''', (email, activation_code, result[0]))
            else:
                cursor.execute('''
                    INSERT INTO trial_info (email, activated, activation_code)
                    VALUES (?, 1, ?)
    ...
    def show_login(self, page: ft.Page):
        """Simplified login - Just Email + Activation Code"""
        page.controls.clear()
        self.page_ref = page
        ...
        saved_user = self.get_saved_user()
        if saved_user:
            print(f"🔐 Auto-login for: {saved_user.get('email')}")
            self.current_user = saved_user
            self.show_dashboard(page)
            return
        ...
        # If trial is active and user has used the app before, auto-login
        if trial_active and self.has_used_app_before():
            self.current_user = {'email': self.get_user_email(), 'trial': True, 'days_left': days_left}
            self.show_dashboard(page)
            return
        ...
        if trial_active:
            ...
            banner_text = "⏰ Trial Expired - Enter Activation Code"
            banner_color = self.danger_color
        
        trial_banner = ft.Container(
            content=ft.Row([
                ft.Icon(ft.icons.INFO_OUTLINE, color="white", size=20),
                ft.Text(banner_text, color="white", size=14, weight=ft.FontWeight.BOLD),
            ], spacing=10),
            padding=12,
        ...
        self.email_field = ft.TextField(
            ...
            bgcolor="#2C2C2C", 
            border_color=self.accent_color,
            value=self.get_user_email() or "",
        )
        
        self.activation_field = ft.TextField(
            label="Activation Code", 
            hint_text="Enter code sent after purchase", 
            width=field_width, 
            bgcolor="#2C2C2C", 
        ...
        # ============================================================
        # FUNCTIONS (All defined BEFORE they're called)
        # ============================================================
        ...
        def start_trial(e):
            ...
            self.current_user = {'email': email, 'trial': True, 'days_left': 30}
            self.save_user_session(self.current_user)  # ✅ Class method
        ...
        def activate_full(e):
            """Activate full version with activation code"""
            email = self.email_field.value.strip()
            activation_code = self.activation_field.value.strip().upper()
            ...
            if not activation_code:
                self.status_text.value = "❌ Please enter activation code!"
                self.status_text.color = self.danger_color
                page.update()
                return
            
            self.loading_indicator.visible = True
            self.status_text.value = "🔄 Verifying activation code..."
            self.status_text.color = self.accent_color
            page.update()
            
            result, message = ActivationManager.verify_activation_code(activation_code)
            
            if result:
                self.loading_indicator.visible = False
                self.status_text.value = "✅ Activation successful! Full access granted."
                self.status_text.color = self.success_color
                
                self.save_activation_info(email, activation_code)
                
                self.current_user = {'email': email, 'activated': True}
                self.save_user_session(self.current_user)  # ✅ Class method
                
                page.snack_bar = ft.SnackBar(
                    ft.Text(f"✅ Full version activated! Welcome!"),
                    bgcolor=self.success_color,
                    duration=3000
                )
                page.snack_bar.open = True
        ...
        activate_btn.on_click = activate_full
        ...
        main_layout = ft.Column([
            ...
            ft.Container(height=10),
            ft.Text("💡 Try 30 days free with your email", size=10, color="#888888"),
            ft.Text("💡 Already purchased? Enter your activation code", size=10, color="#888888"),
        ...
        # ============ SIDEBAR WITH ZOOM ============
    def start_free_trial_demo(self, page: ft.Page):
        """Start free trial with demo credentials - Uses stored UI elements"""
        
        # Check if UI elements exist
        if not hasattr(self, 'email_field') or not hasattr(self, 'password_field'):
            # If not, show login screen first
            self.show_login(page)
            return
        ...
        # Trigger login
        # Find and call the login function
        for child in page.controls:
            if isinstance(child, ft.Container):
                # Find the login button and click it
                for control in child.content.controls:
                    if isinstance(control, ft.Row):
                        for btn in control.controls:
                            if isinstance(btn, ft.FilledButton) and btn.text == "Sign In":
                                btn.on_click(None)
                                return
        
        # Fallback: trigger login directly
        try:
            user = UserManager.authenticate("demo@store.com", "demo123")
            if user:
                user_dict = dict(user)
                company_id = user_dict.get('company_id', 1)
                user_dict['company_id'] = company_id
                ...
                page.snack_bar = ft.SnackBar(
                    ft.Text("✅ Welcome! Your 30-day trial has started."),
                    bgcolor=self.success_color,
                    duration=4000
                )
                page.snack_bar.open = True
                ...
                page.snack_bar = ft.SnackBar(
                    ft.Text("❌ Trial start failed. Please try again."),
                    bgcolor=self.danger_color,
                    duration=3000
                )
                page.snack_bar.open = True
            ...
            page.snack_bar.open = True
            page.update()
    def get_demo_days_left(self, company_id):
            """Get days left in demo for a company"""
            import sqlite3
            from database import DB_PATH
            from datetime import datetime
            ...
            try:
                ...
                print(f"Error getting demo days: {e}")
                return 0
    ...
    def create_sidebar(self, page: ft.Page):
        ...
        def logout(e):
            self.confirm_logout(page)  # <-- Calls confirm_logout
        ...
        logout_btn = ft.Container(
            ...
            border_radius=10,
            ink=True,
            on_click=logout,  # <-- Calls logout function
        )
    ...
    def create_bottom_nav(self, page: ft.Page):
        """Create bottom navigation bar with logout"""
    ...
    def show_account_settings(self, page: ft.Page):
        """Show account settings dialog"""
        
        email = self.current_user.get('email', '')
        activated = self.current_user.get('activated', False)
        ...
        dialog_content = ft.Column([
            ft.Row([
                ft.Text("👤 Account Settings", size=20, weight=ft.FontWeight.BOLD, expand=True),
                ft.IconButton(icon=ft.icons.CLOSE, icon_size=20, on_click=close_dialog),
            ]),
            ft.Divider(),
            ft.Row([
                ft.Icon(ft.icons.EMAIL, color=self.accent_color, size=20),
                ft.Column([
                    ft.Text("Email", size=12, color="#888888"),
            ...
            ft.Row([
                ...
                ft.Column([
                    ft.Text("Status", size=12, color="#888888"),
            ...
            ft.ElevatedButton(
                "📧 Change Email",
                on_click=lambda e: self.show_change_email_dialog(page),
                icon=ft.icons.EDIT,
                expand=True,
            ),
            ft.ElevatedButton(
        ...
        dialog = ft.AlertDialog(
            title=ft.Text(""),
            content=ft.Container(content=dialog_content, width=400, height=420, padding=20),
        )
    ...
    def show_dashboard(self, page: ft.Page):
        """Dashboard with trial banner and activate button"""
        page.controls.clear()
        ...
        if is_activated:
            ...
            top_banner = ft.Container(
                content=ft.Row([
                    ...
                    ft.Column([
                        ...
                        ft.Text(f"Welcome {email}!", size=12, color="#888888"),
                    ], spacing=2, expand=True),
                    ft.ElevatedButton(
                        "🔑 Manage Account",
                        on_click=lambda e: self.show_account_settings(page),
            ...
            top_banner = ft.Container(
                content=ft.Row([
                    ...
                    ft.Column([
                        ft.Text(f"{days_emoji} Free Trial: {days_text}", size=16, weight=ft.FontWeight.BOLD, color="#FF9800"),
            ...
            top_banner = ft.Container(
                content=ft.Row([
                    ...
                    ft.Column([
                        ...
                        ft.Text("Please activate to continue using the app.", size=12, color="#888888"),
                    ], spacing=2, expand=True),
                    ft.ElevatedButton(
                        "🔑 Activate Now",
                        on_click=lambda e: self.show_activation_dialog(page),
        ...
        header_row = ft.Row([
            ...
            # Logout button (only if activated)
            ft.IconButton(
                icon=ft.icons.LOGOUT,
                icon_size=24,
                icon_color="#888888",
        ...
        main_column.controls.append(
            ft.Row([
                ...
                ft.ElevatedButton("📁 Backups", on_click=lambda e: self.show_backup_list(page), expand=True),
            ], spacing=10)
        )
        
        # Row 4 - Activation button (if not activated)
        if not is_activated:
            main_column.controls.append(
                ft.Row([
                    ft.ElevatedButton(
        ...
        status_text = "✅ Full Version" if is_activated else f"🚀 Trial: {days_left} days left" if days_left > 0 else "⚠️ Trial Expired"
    ...
    def _create_stat_card(self, icon, value, label, custom_color=None):
        """Create a statistics card with optional custom color"""
    ...
    def ensure_demo_users_with_codes(self):
        ...
        try:
            ...
            has_temp_password = 'temp_password' in columns
            has_password_changed = 'password_changed' in columns
            
            # Demo users with codes
            demo_users = [
                ('Demo Admin', 'demo@store.com', 'demo123', 'LOGIN-DEMOADM', 'admin'),
                ('Demo Manager', 'manager@store.com', 'demo123', 'LOGIN-MANAGER', 'manager'),
            ...
            for name, email, password, login_code, role in demo_users:
                cursor.execute("SELECT id FROM users WHERE email = ?", (email,))
                if cursor.fetchone():
                    print(f"✅ Demo user exists: {name} ({email})")
                    continue
                ...
                placeholders = ['?', '?', '?', '?', '?', '?']
                values = [name, email, hashed_password, role, company_id, 
                        datetime.now().strftime('%Y-%m-%d %H:%M:%S')]
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO users (
                    ...
                    login_code, code_used, temp_password, password_changed, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                'Administrator',
                'admin@store.com',
    ...
    def auto_sync_after_change(self, page: ft.Page):
        ...
        if self.current_user and self.current_user.get('id', 0) > 0:
            company_id = self.current_user.get('company_id', 1)
            print(f"🔄 Auto-syncing after change for company {company_id}")
    ...
    def open_add_accessory_modal(self, page: ft.Page):
        """Add accessory with image upload and auto-sync to cloud"""
        import random
        import string
        import sqlite3
        import os
    ...
    def generate_login_code(self, user_id, company_id):
        ...
        import time  # <-- OR use this
        
        # Create unique code based on user, company, and timestamp
        raw = f"LOGIN-{user_id}-{company_id}-{int(time.time())}"  # <-- Use time.time()
        hash_obj = hashlib.sha256(raw.encode())
        code = hash_obj.hexdigest()[:8].upper()
    ...
    def migrate_login_code_system(self):
        """Add login code columns to users table"""
        import sqlite3
        from database import DB_PATH
        ...
        try:
            ...
            if 'password_changed' not in columns:
                ...
                print("✅ Added password_changed column")
    ...
    def open_add_modal(self, page: ft.Page):
        """Add material with image upload and auto-sync to cloud"""
        import random
        import string
        import sqlite3
        import os
    ...
    def open_delete_modal(self, page: ft.Page, material_id):
        """Delete confirmation modal with cloud sync notice"""
        ...
        def confirm_delete(e):
            # Delete from local (auto-sync will handle cloud)
            MaterialManager.delete(material_id)
            
            page.dialog.open = False
            page.snack_bar = ft.SnackBar(
                ft.Text(f"✓ Deleted: {name} (syncing to cloud...)"), 
                bgcolor=self.danger_color,
                duration=3000
            )
            page.snack_bar.open = True
    ...
    def show_login_code_dialog(self, page: ft.Page, login_code, email, name, temp_password):
    ...
    How to login:
    1. Download the app
    2. On login screen, enter your Email
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Container(
                content=ft.Column([
                    ...
                    ft.Text("🔒 Temporary Password:", size=13, weight=ft.FontWeight.BOLD),
                    ft.Container(
                        content=ft.Text(temp_password, size=18, weight=ft.FontWeight.BOLD, color="#FF9800"),
                        padding=10,
                        bgcolor="#2C2C2C",
                        border_radius=8,
                        alignment=ft.alignment.center,
                    ),
                    ft.Text("Share this info with the user", size=11, color="#888888"),
                    ft.Row([
                        ft.ElevatedButton(
                            "📋 Copy Code",
                            on_click=copy_code,
                    ...
                    ft.Text("4. Enter the temporary password", size=10, color="#888888"),
                    ft.Text("5. Set your new password", size=10, color="#888888"),
                    ft.Text("6. Start using the app!", size=10, color="#888888"),
                ], spacing=10),
                width=450,
                height=560,
                padding=20,
    ...
    def open_add_user_modal(self, page: ft.Page):
        """Open modal for adding new user with login code and temp password"""
        ...
        def save_user(e):
            ...
            # Generate temp password and login code
            import random
            import string
            import hashlib
            from datetime import datetime  # <-- ADD THIS IMPORT
            ...
            # ===== FIX: Use time.time() instead of datetime.now().timestamp() =====
            login_code = self.generate_login_code(
                int(time.time()),  # <-- FIXED: Use time.time()
                company_id
            )
            
            import sqlite3
            from database import DB_PATH
            from datetime import datetime  # <-- Also need this for created_at
            ...
            try:
                ...
                self.show_login_code_dialog(page, login_code, email, name, temp_password)
                ...
                content=ft.Container(
                    content=ft.Column([
                        ...
                        ft.Column([
                            ...
                            role_field,
                            status_text,
                            ft.Text("💡 User will receive a login code and temporary password", size=11, color="#888888"),
                        ], spacing=12),
                        ft.Divider(),
                        ft.Row([
        ...
        def confirm_delete(e):
            ...
            try:
                ...
                def sync_after_delete():
                    try:
                        # Sync all remaining users to cloud (which removes the deleted one)
                        CloudSyncManager.sync_users_to_cloud(company_id)
    ...
    def confirm_logout(self, page: ft.Page):
        ...
        dialog_content = ft.Column([
            ...
            ft.Divider(),
            ft.Text("Are you sure you want to logout?", size=14),
            ft.Text("You can always continue with the free trial.", size=12, color="#888888"),
            ft.Container(height=15),
            ft.Row([
                ft.TextButton("Cancel", on_click=cancel_logout),
